...
public class GvrPointerScrollInput
{
    ...
    private float SPEED_CLAMP;
    private float SPEED_CLAMP_SQUARED;
    private const float INERTIA_THRESHOLD_RATIO = 0.2f;
    private float INERTIA_THRESHOLD ;
    private float INERTIA_THRESHOLD_SQUARED;
    private  float SLOP_VERTICAL;
    private  float SLOP_HORIZONTAL;
    ...
    private void CalculationValue(float scrollerDalta)
    {
        SPEED_CLAMP = (SPEED_CLAMP_RATIO * scrollerDalta);
        SPEED_CLAMP_SQUARED = SPEED_CLAMP * SPEED_CLAMP;
        INERTIA_THRESHOLD = (INERTIA_THRESHOLD_RATIO * scrollerDalta);
        INERTIA_THRESHOLD_SQUARED = INERTIA_THRESHOLD * INERTIA_THRESHOLD;
        SLOP_VERTICAL = 0.165f * scrollerDalta;
        SLOP_HORIZONTAL = 0.15f * scrollerDalta;
    }
    ...
    private ScrollInfo AddScrollHandler(GameObject scrollHandler, Vector2 currentScroll)
    {
        ...
        if (scrollInfo.scrollSettings != null)
        {
            scrollInfo.initScroll = currentScroll * scrollInfo.scrollSettings.ScrollDeltaMultiplier;
            scrollInfo.lastScroll = currentScroll * scrollInfo.scrollSettings.ScrollDeltaMultiplier;
        }
        else
        {
            scrollInfo.initScroll = currentScroll * SCROLL_DELTA_MULTIPLIER;
            scrollInfo.lastScroll = currentScroll * SCROLL_DELTA_MULTIPLIER;
        }
    ...
    private  bool CanScrollStartX(ScrollInfo scrollInfo, Vector2 currentScroll)
    {
        if (scrollInfo == null)
        {
            return false;
    ...
    private float GetScrollDeltaMultiplier(ScrollInfo scrollInfo)
    {
        if (scrollInfo != null && scrollInfo.scrollSettings != null)
        {
            return scrollInfo.scrollSettings.ScrollDeltaMultiplier;
        }

        return SCROLL_DELTA_MULTIPLIER;
    }
    private  bool CanScrollStartY(ScrollInfo scrollInfo, Vector2 currentScroll)
    {
        if (scrollInfo == null)
        {
            return false;
